...
class CS2Router {
        ...
        try {
                ...
                case 'analytics':
                    ...
                    this.pages.analytics = {
                        render: (container) => {
                            ...
                            placeholder.className = 'text-center py-20';
                            
                            const title = document.createElement('h1');
                            title.className = 'text-3xl font-bold text-white mb-4';
                            title.textContent = 'Advanced Analytics';
                            placeholder.appendChild(title);
                            
                            const message = document.createElement('p');
                            message.className = 'text-gray-400';
                            message.textContent = 'Market intelligence features coming soon...';
            ...
            this.pages[pageName] = {
                render: (container) => {
                    ...
                    title.textContent = `Error loading ${pageName}`;
                    errorDiv.appendChild(title);
                    
                    const message = document.createElement('p');
                    message.className = 'text-gray-400';
                    message.textContent = error.message || 'Module failed to load';
    ...
    async displayPage(pageName, params) {
        ...
        if (this.pages[pageName] && typeof this.pages[pageName].render === 'function') {
            ...
            // Clear and create not found message
            pageElement.textContent = '';
            const notFoundDiv = document.createElement('div');
            notFoundDiv.className = 'text-center py-20';
            
            const title = document.createElement('h1');
            title.textContent = `Page not found: ${pageName}`;
            notFoundDiv.appendChild(title);
            
            pageElement.appendChild(notFoundDiv);
        }
    ...
    showErrorPage(error) {
        ...
        if (mainContent) {
            ...
            const title = document.createElement('h1');
            title.className = 'text-3xl font-bold text-red-400 mb-4';
            title.textContent = 'Navigation Error';
            errorContainer.appendChild(title);
            
            const message = document.createElement('p');
            message.className = 'text-gray-400';
            message.textContent = 'Failed to navigate to the requested page';
